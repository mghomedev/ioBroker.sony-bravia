...
class Bravia {
    ...
    getSoundSettingsTargetSystemOrNull() {
        return new Promise(resolve => {
            try {
                this.getSoundSettings().then(resp => {
                    if (Array.isArray(resp)) {
                        for (var r in resp) {
                            if (r.target == "outputTerminal") resolve(r.currentValue);
                        }
                        resolve(resp[0].currentValue);
                    } else {
                        resolve(resp.currentValue);
                    }
                }, rej => {
                    resolve("error crash:" + rej);
                });
            } catch (e) {
                resolve("error:" + e);
            }
    ...
    setAudioVolumeInformation(audioTarget, desiredVolume) {
        return new Promise((resolve, reject) => {
            try {
                this.audio
                    .invoke('setAudioVolume', "1.0", {
                        target: audioTarget,
